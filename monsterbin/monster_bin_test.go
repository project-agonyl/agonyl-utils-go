...
func TestRead_SingleItem(t *testing.T) {
	item := MonsterBinItem{ID: 1}
	copy(item.Name[:], "Goblin")

	var buf bytes.Buffer
	require.NoError(t, Write(&buf, MonsterBin{item}))

	data, err := Read(&buf)
	require.NoError(t, err)
	require.Len(t, data, 1)
	assert.Equal(t, uint32(1), data[0].ID)
	assert.Equal(t, "Goblin", data[0].GetName())
}
...
func TestRead_MultipleItems(t *testing.T) {
	...
	copy(items[0].Name[:], "Goblin")
	copy(items[1].Name[:], "Orc")
	copy(items[2].Name[:], "Dragon")
	...
	assert.Equal(t, "Orc", data[1].GetName())
	assert.Equal(t, uint32(3), data[2].ID)
	assert.Equal(t, "Dragon", data[2].GetName())
}
...
func TestRead_TruncatedItem(t *testing.T) {
	// Count = 1 but stream ends before full item (need 4 + 0x1F + 0x3D = 100 bytes after count)
	var buf bytes.Buffer
	count := []byte{0x01, 0x00, 0x00, 0x00}
	buf.Write(count)
	buf.Write(bytes.Repeat([]byte{0}, 50)) // only 50 bytes, need 100
...
func TestWriteThenRead_RoundTrip(t *testing.T) {
	...
	original[0].Unknown[0] = 0xAB
	original[1].Unknown[1] = 0xCD
...
func TestGetName_ShortName(t *testing.T) {
	var m MonsterBinItem
	copy(m.Name[:], "Goblin")
	assert.Equal(t, "Goblin", m.GetName())
}
...
func TestGetName_NoNullInBuffer(t *testing.T) {
	...
	name := m.GetName()
	assert.Len(t, name, 31)
	assert.Equal(t, "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX", name)
}
